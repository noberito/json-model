...
from .analyze import disjunct_analyse, distinct_prop_objects, ultimate_type
from .irep import IRep, evaluate, optimizeIR
from .language import Block, BoolExpr, Code, JsonExpr, Language, PathExpr, PropMap, StrExpr, Var
...
class CodeGenerator:
    ...
    def _compileConstraint(
        ...
        # initial model check
        code: Block = self._compileModel(
            jm, model["@"], mpath + ["@"], res, val, vpath, constrained=len(cmp_props) > 0
        )
        ...
        if tmodel is None:
            ...
            if has_int:
                if tmodel in (int, str, list, dict):
                    ...
                    fval = gen.ident("fval")
                    cvars += gen.flt_var(
                        fval, gen.value(val, Number if jm._loose_float else float), declare=True
                    )  # type: ignore
            if has_flt:
        ...
        code += gen.if_stmt(
            res,
            cvars
            + gen.bool_var(res, gen.and_op(*checks))
    ...
    def _disjunction(
        ...
        for tag_name, tag_type, lalts in disjuncts:
            ...
            if isobj_needed:
                tag_code += gen.if_stmt(
                    res, tagt, gen.report(f"value is not an object [{smpath}]", vpath), likely=True
                )
            else:
                tag_code += tagt
    ...
    def _closeMuObject(
        ...
        code = (
            ...
            gen.if_stmt(
                gen.num_cmp(gen.obj_len(val), "!=", gen.const(len(must))),
        ...
        for prop, pmodel in must.items():
            ...
            if pmodel != "$ANY":
                ...
                code += self._compileModel(
                    jm, pmodel, mpath + [prop], res, "pval", lpath_ref
                ) + gen.if_stmt(
                    gen.not_op(res),
                    self._gen_fail(
                        f"unexpected value for mandatory prop <{prop}> [{smpath}]", lpath_ref
                    ),
                    likely=False,
                )
    ...
    def _openMuMaObject(
        ...
        code = gen.lcom("check open must/may only props") + gen.if_stmt(
            gen.not_op(gen.is_a(val, dict)),
            self._gen_fail(f"not an object [{smpath}]", vpath),
            likely=False,
        )
        ...
        for prop, pmodel in may.items():
            if pmodel != "$ANY":
                ...
                check_value += self._compileModel(
                    jm, pmodel, mpath + [prop], res, "pval", lpath_ref
                ) + gen.if_stmt(
                    gen.not_op(res),
                    self._gen_fail(
                        f"unexpected value for optional prop <{prop}> [{smpath}]", lpath_ref
                    ),
                    likely=False,
                )
    ...
    def _compileObject(
        ...
        if (
            ...
            and not regs
            and not oth
            and len(must) <= self._must_only_threshold
        ):
        ...
        if (
            ...
            or oth
            and oth[""] != "$ANY"
            or 1 <= len(must) <= self._map_threshold
            or 1 <= len(may) <= self._map_threshold
        ):
        ...
        if must:
            ...
            if len(must) <= self._map_threshold:  # unroll property checks
                ...
                if partitioned:
                    ...
                    body_code += gen.hash_var(
                        hash_var, gen.str_hash(prop, hash_size), declare=True
                ...
                if gen.assign_expr():
                    ...
                    mu_code = gen.lcom(f"handle {len(must)} mandatory props") + gen.if_stmt(
                        gen.is_def(pfun), mu_prop_code, likely=likely
                    )
                else:
            ...
            if len(may) <= self._map_threshold:  # simple few-property code
                ...
                if partitioned:
                    ...
                    if declare:
                        hash_var = gen.ident("hash")
                    body_code += gen.hash_var(
                        hash_var, gen.str_hash(prop, hash_size), declare=declare
                    ) + self._gen_parts(hash_var, part_code)
                else:
                    # accumulate in global multi_if
        ...
        for d, m in defs.items():
            ...
            dl_code = (
                gen.lcom(f"handle {len(defs)} key props")
        ...
        for r, v in regs.items():
            ...
            rg_code = (
                gen.lcom(f"handle {len(regs)} re props")
        ...
        if must:
            ...
            code += gen.if_stmt(
                gen.num_cmp(must_c, "!=", gen.const(len(must))),
                self._gen_reporting(missing) + gen.ret(gen.false()),
                likely=False,
            )
    ...
    def _compileOr(
        ...
        if len(list(filter(lambda t: t[0], l_const))) >= 2:
            ...
            if constants:
                ...
                code += gen.bool_var(
                    res, gen.and_op(gen.is_scalar(val), gen.in_cset(sname, val, constants))
                )
        ...
        mandatory_props: list[tuple[str, bool] | None] = (
            ...
            if same
            and expected is dict
            and self._or_must_prop
            and len(models) >= self._or_must_prop
            ...
            if mandatory_props[i]:
                ...
                if full:
                    ...
                    icode = gen.if_stmt(
                        cond,
                        m_code,
                        icode if icode else gen.bool_var(res, gen.false()),
                        likely=len(icode) == 0,
                    )
                else:
    ...
    def _compileXor(
        ...
        if len(models) == 0:
            code += gen.lcom("early empty xor list") + gen.bool_var(res, gen.false())
            return code

        # NOTE optimized out under -O
        if len(models) == 1:
            return gen.lcom("early singleton xor list") + self._compileModel(
                jm, models[0], mpath + [0], res, val, vpath, known
            )
        ...
        if len(models) >= 2:
            ...
            if dups:
                ...
                for i, m in enumerate(dups):
                    idx = dups_i[i]
                    icode = self._compileModel(
                        jm, m, mpath + [idx], isin, val, vpath
        ...
        if not models:
            ...
            xcode += gen.lcom("singleton xor list") + self._compileModel(
                jm, mod, mpath + [idx], res, val, vpath
            )
    ...
    def _compileAnd(
        ...
        for i, m in reversed(list(enumerate(models))):
            acode = gen.if_stmt(
                res, self._compileModel(jm, m, mpath + [i], res, val, vpath, and_known) + acode
            )
        code += acode
    ...
    def _compileModel(
        ...
        match model:
            case None:
                code += gen.bool_var(res, gen.is_a(val, None)) + self._gen_report(
                    res, f"not null [{smpath}]", vpath
                )

            case bool():
                code += gen.bool_var(res, gen.is_a(val, bool)) + self._gen_report(
                    res, f"not a bool [{smpath}]", vpath
                )
            ...
            case int():
                ...
                if expr:
                    ...
                    code += gen.bool_var(res, expr) + self._gen_report(
                        res, f"not a {model} {looseness} int [{smpath}]", vpath
                    )
            ...
            case float():
                ...
                if expr:
                    ...
                    code += gen.bool_var(res, expr) + self._gen_report(
                        res, f"not a {model} {looseness} float [{smpath}]", vpath
                    )
                ...
                if len(model) == 0:  # []
                    ...
                    if model[0] == "$ANY":
                        ...
                        loop = gen.arr_loop(
                            ...
                            + self._compileModel(
                                jm, model[0], mpath + [0], res, item, gen.path_lvar(lpath, vpath)
                            )  # type: ignore
                    ...
                    code += (
                        ...
                        + gen.if_stmt(res, loop, likely=True)
                        if expr
                        else loop
                    )
                    ...
                    if not constrained:
                        ...
                        for i, m in reversed(list(enumerate(model[:-1]))):
                            ith_check = gen.if_stmt(
                                gen.num_cmp(arlen, ">", i),
                                gen.path_var(lpath, gen.path_val(vpath, i, False, False))
                                + self._compileModel(
                                    jm, m, mpath + [i], res, gen.arr_item_val(val, i), lpath_ref
                                ),
                            )
                            if body:
                        ...
                        code += (
                            gen.bool_var(res, expr if expr else gen.true())
                            + gen.if_stmt(res, body, likely=True)
                            if expr
                            else body
                        )
    ...
    def _compileName(
        self, jm: JsonModel, name: str, model: ModelType, mpath: ModelPath, local: bool = False
    ):
        ...
        if is_a_simple_object(model):
            ...
            body = (
                self._lang.bool_var("res", declare=True)
...
def xstatic_compile(
    ...
    *,
    ...
    MUST_ONLY_THRESHOLD: dict[str, int] = {
        ...
        "pl": 128,  # ?
        "java": 256,  # GSON
        "go": 256,  # Similar to Java/JS, map overhead vs unroll
    ...
    MAY_MUST_OPEN_THRESHOLD: dict[str, int] = {
        ...
        "go": 16,  # Go maps are reasonably fast, similar config to python/js
        "sql": 0,  # FIXME not tested
        "plpgsql": 0,  # FIXME not tested
    }
    ...
    log.info(
        f"lang={lang} mt={map_threshold} mmo={may_must_open_threshold} "
